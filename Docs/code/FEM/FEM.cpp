...
void FEM::SolveElliptical() {
    A = new GlobalMatrix(); b = new GlobalVector(_generatedRibs.size());
...
void FEM::BuildMatrixAndVector() {
    switch (Type) {
    case EquationType::Hyperbolical: SolveHyperbolical(); break;
    case EquationType::Parabolical: SolveParabolical(); break;
    case EquationType::Elliptical: SolveElliptical(); break;
    case EquationType::NotStated: default:
...
void FEM::SetSolver(Solver* s) { _s = s;}

void FEM::Solve() { x = _s->Solve(*A, *b);}

void FEM::WriteAnswer() {
    std::ofstream fout("Data/Output/solution.txt");
    for (size_t i(0); i < x.getSize(); ++i) 
        fout << i << ". " << std::setprecision(15) << std::scientific << x(i) << std::endl;
    fout.close(); }

FEM::FEM() { Logger::ConsoleOutput("FEM declared, but it's empty", NotificationColor::Warning); }
