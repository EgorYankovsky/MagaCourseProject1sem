...
void MeshGenerator::Generate3DMesh(Mesh& mesh) {
    ...
    GenerateListOfPoints(mesh); GenerateListOfRibs(mesh);
    GenerateListOfAreas(mesh); GenerateListOfBorders(mesh);
}
...
int MeshGenerator::SelectAreaNum(Mesh& mesh, std::array<size_t, 12> arr) {
    ...
    auto lz0 = p1 / sxy; auto lz1 = p2 / sxy;
    for (const auto& area : mesh.subdomains_) 
        if (mesh.numRefsOfLinesAboveX[area[1]] <= lx0 and 
            lx1 <= mesh.numRefsOfLinesAboveX[area[2]] and
            mesh.numRefsOfLinesAboveY[area[3]] <= ly0 and 
            ly1 <= mesh.numRefsOfLinesAboveY[area[4]] and
            mesh.numRefsOfLinesAboveZ[area[5]] <= lz0 and 
            lz1 <= mesh.numRefsOfLinesAboveZ[area[6]])
            return area[0];
...
void MeshGenerator::GenerateListOfPoints(Mesh& mesh) {
        ...
        for (size_t j(0); j < mesh.LinesAmountY; ++j) {
            ...
            for (const auto& info : mesh.delimitersX_) {
                ...
                *iterOnXRefs = *(iterOnXRefs - 1) + amountOfDelimiters;
                ...
                double x0 = (*(rightBorderIter - 1)).x; double x1 = (*rightBorderIter).x;
                double y0 = (*(rightBorderIter - 1)).y; double y1 = (*rightBorderIter).y;
                double z0 = (*(rightBorderIter - 1)).z; double z1 = (*rightBorderIter).z;
                double deltX = x1 - x0; double deltY = y1 - y0; double deltZ = z1 - z0;
                double xh = deltX / denum; double yh = deltY / denum;
                double zh = deltZ / denum; double multiplier = 0.0;
                for (size_t ii(0); ii < amountOfDelimiters - 1; ++ii) {
                    ...
                    auto pointToInsert = Point(x0 + xh * multiplier, 
                        y0 + yh * multiplier, z0 + zh * multiplier);
    ...
    for (size_t k(0); k < mesh.LinesAmountZ; ++k) {
        ...
        for (const auto& info : mesh.delimitersY_) {
            ...
            *iterOnYRefs = *(iterOnYRefs - 1) + amountOfDelimiters;
            iterOnYRefs++;
            line1D v0(mesh.LinesAmountX); line1D v1(mesh.LinesAmountX);
            ...
            for (size_t ii(0); ii < amountOfDelimiters; ++ii)
                denum += pow(coefficientOfDelimiter, ii);
            for (size_t i(0); i < mesh.LinesAmountX; ++i) {
                double x0 = v0[i].x; double x1 = v1[i].x;
                double y0 = v0[i].y; double y1 = v1[i].y;
                double z0 = v0[i].z; double z1 = v1[i].z;
                double deltX = x1 - x0; double deltY = y1 - y0; double deltZ = z1 - z0;
                double xh = deltX / denum; double yh = deltY / denum;
                double zh = deltZ / denum; double multiplier = 0.0;
                for (size_t ii(0); ii < amountOfDelimiters - 1; ++ii) {
    ...
    for (const auto& info : mesh.delimitersZ_) {
        ...
        *iterOnZRefs = *(iterOnZRefs - 1) + amountOfDelimiters;
        ...
        for (size_t j(0); j < mesh.LinesAmountY; ++j) {
            for (size_t i(0); i < mesh.LinesAmountX; ++i) {
                double x0 = s0[j][i].x; double x1 = s1[j][i].x;
                double y0 = s0[j][i].y; double y1 = s1[j][i].y;
                double z0 = s0[j][i].z; double z1 = s1[j][i].z;
                double deltX = x1 - x0; double deltY = y1 - y0;
                double deltZ = z1 - z0;
...
void MeshGenerator::GenerateListOfAreas(Mesh& mesh) {
    size_t rx = mesh.LinesAmountX - 1; size_t ry = mesh.LinesAmountY - 1;
    size_t nx = mesh.LinesAmountX; size_t ny = mesh.LinesAmountY;
    size_t rxy = rx * ny + ry * nx; size_t nxy = nx * ny;
...
void MeshGenerator::GenerateListOfRibs(Mesh& mesh) {
    auto nx = mesh.LinesAmountX; auto ny = mesh.LinesAmountY;
...
void MeshGenerator::GenerateListOfBorders(Mesh& mesh) {
    ...
    auto nx = mesh.getLinesAmountX(); auto ny = mesh.getLinesAmountY();
    auto nz = mesh.getLinesAmountZ(); auto nxny = nx * ny;
    auto rxy = (nx - 1) * ny + (ny - 1) * nx;
    // XY0
    for (size_t i = 0; i < ny - 1; i++)
        for (size_t j = 0; j < nx - 1; j++)
            mesh.newBorders_.push_back(std::array<size_t, 6> {1, 1, 
                i * (2 * nx - 1) + j, i * (2 * nx - 1) + j + nx - 1,
                i * (2 * nx - 1) + j + nx, i * (2 * nx - 1) + j + nx + nx - 1});
    // X0Z
    for (size_t i = 0; i < nz - 1; i++)
        for (size_t j = 0; j < nx - 1; j++)
            mesh.newBorders_.push_back(std::array<size_t, 6> {1, 1, 
                i * (rxy + nxny) + j, i * (rxy + nxny) + j + rxy,
                i * (rxy + nxny) + j + rxy + 1, i * (rxy + nxny) + j + rxy + nxny});
    // 0YZ
    for (size_t i = 0; i < nz - 1; i++)
        for (size_t j = 0; j < ny - 1; j++)
            mesh.newBorders_.push_back(std::array<size_t, 6> {1, 1, 
                nx - 1 + i * (rxy + nxny) + j * (2 * nx - 1), rxy + i * (rxy + nxny) + j * nx,
                rxy + nx + i * (rxy + nxny) + j * nx, rxy + nxny + nx - 1 + i * (rxy + nxny) + j * (2 * nx - 1)});
    // XY1
    for (size_t i = 0; i < ny - 1; i++)
        for (size_t j = 0; j < nx - 1; j++)
            mesh.newBorders_.push_back(std::array<size_t, 6> {1, 1, 
                (nz - 1)* (rxy + nxny) + j + i * (2 * nx - 1),
    ...
    for (size_t i = 0; i < nz - 1; i++)
        for (size_t j = 0; j < nx - 1; j++)
            mesh.newBorders_.push_back(std::array<size_t, 6> {1, 1, 
                (ny - 1)* nx + (ny - 1) * (nx - 1) + j + i * (rxy + nxny),
